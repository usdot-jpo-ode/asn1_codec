...
main(int ac, char *av[]) {
    ...
    // OER support (function pointers) are disabled using the compilation flag: -DASN_DISABLE_OER_SUPPORT
    // PER support (function pointers) are disabled using the compilation flag: -DASN_DISABLE_PER_SUPPORT
    // CAUTION: OER is the default if you do not disable any.

    if(pduType->op->oer_decoder) {

        // we have an oer decoder function, so default to that input.
        fprintf( stderr, "JMC: Input type is INP_OER\n" );
        iform = INP_OER;

    } else if(pduType->op->uper_decoder) {

        // we have an per decoder function, so default to that input.
        // For j2735 we are using this default.
        fprintf( stderr, "JMC: Input type is INP_UPER\n" );
        iform = INP_PER;
    }
     ...
     *
     * -i (input) has an argument: per, ber, xer
     * -o (output) has an argument: per, der, xer
     * -1 no argument; decode only the first PDU.
     * -b (buffer size) has an numeric argument to set the buffer size.
     * -c (check ASN.1 constraints) no argument.
     * -d (debugging on) no argument.
     * -n (processing iterations). argument numberic.
     * -p (assume per PDUs are not padded) argument er-nopad
     * -h (help) no argument.
     * -s (stack limit) argument needed; the size of stack other than default.
     */
    while((ch = getopt(ac, av, "i:o:1b:cdn:p:hs:" JUNKOPT)) != -1) {
        ...
        switch(ch) {
            case 'i':
                ...
                // just using first letter to distinguish input types.
                if(optarg[0] == 'b') { iform = INP_BER; break; }
                if(optarg[0] == 'x') { iform = INP_XER; break; }
                if(pduType->op->oer_decoder && optarg[0] == 'o') { iform = INP_OER; break; }
                if(pduType->op->uper_decoder && optarg[0] == 'p') { iform = INP_PER; break; }
            ...
            case 'o':
                ...
                if(pduType->op->oer_encoder && optarg[0] == 'o') { oform = OUT_OER; break; }
                if(pduType->op->uper_encoder && optarg[0] == 'p') { oform = OUT_PER; break; }
            ...
            case '1':
                ...
                // only process this many PDUs from the input.
                opt_onepdu = 1;
                break;
            ...
            case 'b':
                ...
                if(suggested_bufsize < 1 || suggested_bufsize > 16 * 1024 * 1024) {
                    fprintf(stderr, "-b %s: Improper buffer size (1..16M)\n", optarg);
                    exit(EX_UNAVAILABLE);
                }
                break;
            ...
            case 'n':
                ...
                if(number_of_iterations < 1) {
                    fprintf(stderr, "-n %s: Improper iterations count\n", optarg);
                    exit(EX_UNAVAILABLE);
                }
                break;
...
#define    _ASX(x)    _AXS(x)
                ...
                if(pduType->op->oer_decoder) {
                    fprintf(stderr, "  -ioer        Input is in OER (Octet Encoding Rules)%s\n", iform == INP_OER ? " (DEFAULT)" : "");
                }

                if(pduType->op->uper_decoder) {
                    fprintf(stderr, "  -iper        Input is in Unaligned PER (Packed Encoding Rules)%s\n", iform == INP_PER ? " (DEFAULT)" : "");
                }

                fprintf(stderr, "  -iber        Input is in BER (Basic Encoding Rules)%s\n", iform == INP_BER ? " (DEFAULT)" : "");

                fprintf(stderr, "  -ixer        Input is in XER (XML Encoding Rules)\n");

                if(pduType->op->oer_encoder) {
                    fprintf(stderr, "  -ooer        Output in Canonical OER (Octet Encoding Rules)\n");
                }

                if(pduType->op->uper_encoder) {
                    fprintf(stderr, "  -oper        Output in Unaligned PER (Packed Encoding Rules)\n");
                }
    ...
    ac -= optind;               // the number of remaining arguments.
    av += optind;               // advance the pointer to arguments to the next one.
    ...
    // fprintf( stdout, "JMC: going to process files...\n" );

    for(num = 0; num < number_of_iterations; num++) {
      int ac_i;

      // process the remaining operands (files to decode)
      for(ac_i = 0; ac_i < ac; ac_i++) {
        ...
        // BL: always go thru one time; if processing the first one only, jump out after that is complete.
        for(first_pdu = 1; first_pdu || !opt_onepdu; first_pdu = 0) {
            /*
             * Decode the encoded structure from file.
             */
            // fprintf( stdout, "JMC: going to decode data.\n" );
            structure = data_decode_from_file(pduType, file, name, suggested_bufsize, first_pdu);
            if(!structure) {
                if(errno) {
            ...
            switch(oform) {
                ...
                case OUT_DER: /* : distinguished decoding rules */
                    // fprintf( stdout, "JMC: OUT_DER\n" );
                ...
                case OUT_OER:  /* : octet encoding rules */
                    // fprintf( stdout, "JMC: OUT_OER\n" );
                ...
                case OUT_PER:  /* : packed encoding rules, UPER = unaligned PER, CPER = canonical PER. */
                    // fprintf( stdout, "JMC: OUT_PER\n" );
#ifdef  ASN_DISABLE_PER_SUPPORT
                    erv.encoded = -1;
#else
                    // fprintf( stdout, "JMC: getting ready to encode into XML.\n" );
                    erv = uper_encode(pduType, structure, write_out, binary_out);
#endif
                    if(erv.encoded < 0) {
                        fprintf(stderr, "%s: Cannot convert %s into Unaligned PER\n", name, pduType->name);
                        exit(EX_UNAVAILABLE);
                    }
...
// #endif
// 
//         DEBUG("ofp %d, no=%ld, oo=%ld, dbl=%ld", on_first_pdu, (long)new_offset, (long)old_offset, (long)DynamicBuffer.length);
//         fprintf(stderr, "%s: "
...
//         }
// 
//         fprintf(stderr, "Decoding %ld bytes\n", (long)i_size);
//         DEBUG("Decoding %ld bytes", (long)i_size);
// 
// #ifdef    JUNKTEST
...
//         switch(iform) {
//         case INP_BER:
//             fprintf(stderr,"INP_BER\n");
//             rval = ber_decode(opt_codec_ctx, pduType,
//                 (void **)&structure, i_bptr, i_size);
//             break;
//         case INP_OER:
//             fprintf(stderr,"INP_OER\n");
// #ifdef ASN_DISABLE_OER_SUPPORT
//             rval.code = RC_FAIL;
//             rval.consumed = 0;
// #else
...
//             break;
//         case INP_XER:
//             fprintf(stderr,"INP_XER\n");
//             rval = xer_decode(opt_codec_ctx, pduType,
//                 (void **)&structure, i_bptr, i_size);
//             break;
//         case INP_PER:
//             fprintf(stderr,"INP_PER\n");
// #ifdef ASN_DISABLE_PER_SUPPORT
//             rval.code = RC_FAIL;
//             rval.consumed = 0;
// #else
